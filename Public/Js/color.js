...
color_htm += '<table cellpadding="0" cellspacing="0" bgcolor="#427FB8" width="160">';
color_htm += '<tr><td width="2" height="20"> </td>';
color_htm += '<td><input type="text" style="width:60px;height:12px;border:#CCCCCC 1px solid;line-height:12px;" value="" id="color_viewview" onblur="color_select(this.value);" onkeyup="color_view(this.value);" ondblclick="this.value=\'\';"/></td>';
color_htm += '<td>&nbsp;&nbsp;&nbsp;</td>';
color_htm += '<td align="right" style="color:#FFFFFF;font-weight:bold;cursor:pointer;" onclick="color_close();" title="Close">&#215;&nbsp;</td>';
color_htm += '</tr>';
color_htm += '</table>';
color_htm += '<div id="thinklc_color_show"></div>';
...
function color_show(id, color, obj) {
	Eh();
	if(Dd('thinklc_color') == null) {
		var thinklc_color_div = document.createElement("div");
		with(thinklc_color_div.style) {zIndex = 9999; position = 'absolute'; display = 'none'; width = '160px'; padding = '1px'; top = 0; left = 0; border = '#CCCCCC 1px solid'; backgroundColor = '#FFFFFF';}
		thinklc_color_div.id = 'thinklc_color';
		document.body.appendChild(thinklc_color_div);
	}
	var aTag = obj; var leftpos = toppos = 0;
	do {aTag = aTag.offsetParent; leftpos	+= aTag.offsetLeft; toppos += aTag.offsetTop;
	} while(aTag.offsetParent != null);
	Dd('thinklc_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	Dd('thinklc_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	Dd('thinklc_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	Dd('thinklc_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {Dh('thinklc_color'); Es();}
...
function color_setup(color) {
	...
	colors_select += '</table>';
	Dd('thinklc_color_show').innerHTML = colors_select;
}
