...
function getModule() {
	...
	foreach($module as $k => $v) {
		if($v['status'] && !empty($v['mname'])) $modules[$k] = $v;
	}
	return $modules;
}
//获取置顶图片
...
function getGrade($amount){
	...
	$picstr = '';
	$jb = $jibie;
	if($jb>=$top) $picstr = '<img src="__PUBLIC__/Images/vip/top.gif" border="0" align="absmiddle" alt="信誉级别：至尊商家">';
	else {
		if($jb>=64) {
			$tmp = (int)($jb/64);
