...
function show_license() {
	...
	} elseif(strtoupper(md5($license)) != S_LICENSE) {

		show_msg('license_contents', lang('license_error'), 1);
...
function createtable($sql) {
	//if(mysql_get_server_info() <= '4.1') return str_replace('ENGINE=MyISAM DEFAULT CHARSET=utf8','TYPE=MyISAM',$sql);
	//else return $sql;
...
<div class="container">
	<div class="header">
		<h1>$title</h1>
		<span>ThinkLC&nbsp; V$version &nbsp; $install_lang &nbsp; R$release</span>
EOT;
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2012 <a href="http://www.life0573.com/">ThinkLC</a> Inc.</div>
	</div>
</div>
</body>
</html>
...
function save_config_file($filename, $config) {
	$d_filename = ROOT_PATH.'/config.default.php';
	$content = file_get_contents($d_filename);
	foreach($config as $k=>$v)	{
		$content = preg_replace("/'".$k."'\s*\=\>\s*'.*?'\,/is", "'".$k."' => '".$v."',", $content);
	}
	file_put_contents($d_filename, $content);
	rename($d_filename, $filename);
}
...
function check_adminuser($username, $password, $email) {
	...
	include ROOT_PATH.'./api/uc_client/client.php';
...
function save_uc_config($config, $file) {
	...
	$year = date('Y');
	$config = array(
	'UC_CONNECT' => $uc_connnect,
	'UC_DBHOST' => $ucdbhost,
	'UC_DBUSER' => $ucdbuser,
	'UC_DBPW' => $ucdbpw,
	'UC_DBNAME' => $ucdbname,
	'UC_DBCHARSET' => $ucdbcharset,
	'UC_DBTABLEPRE' => '`'.$ucdbname.'`.'.$uctablepre,
...
function install_ucconfig() {
	$uc_config_file = ROOT_PATH.'./Data/ucconfig.php';
	if(file_exists($uc_config_file)) {
		$uc_config = include $uc_config_file;
		if(is_array($uc_config) && !empty($uc_config)) {
			$cookiepre = 'c'.substr(md5($_SERVER['SERVER_ADDR'].$_SERVER['HTTP_USER_AGENT'].substr($timestamp, 0, 4)), 8, 6).'_';
			if(!empty($uc_config['UC_CONNECT'])) $uc_mysql = '1';
			else $uc_mysql = '0';
			$sql = "INSERT INTO thinklc_setting (item, item_key, item_value) VALUES
('1', 'uc_cookie_pre', '".$cookiepre."'),
('1', 'uc_dbcharset', '".$uc_config['UC_DBCHARSET']."'),
('1', 'uc_dbname', '".$uc_config['UC_DBNAME']."'),
('1', 'uc_dbtablepre', '".$uc_config['UC_DBTABLEPRE']."'),
('1', 'uc_dbpw', '".$uc_config['UC_DBPW']."'),
('1', 'uc_dbuser', '".$uc_config['UC_DBUSER']."'),
('1', 'uc_dbhost', '".$uc_config['UC_DBHOST']."'),
('1', 'uc_mysql', '".$uc_mysql."'),
('1', 'uc_key', '".$uc_config['UC_KEY']."'),
('1', 'uc_appid', '".$uc_config['UC_APPID']."'),
('1', 'uc_ip', '".$uc_config['UC_IP']."'),
('1', 'uc_api', '".$uc_config['UC_API']."'),
('1', 'uc_verify', '1'),
('1', 'uc_enable', '1');";
			@unlink($uc_config_file);
			runquery($sql);
		}
	}
}
