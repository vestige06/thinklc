...
class InfoModel extends CommonModel {
	...
	protected function getexpired() {
		if(empty($_POST['expired']) || !$_POST['expired']) return false;
		return strtotime($_POST['expired'].' 23:59:59');
	}
	protected function getlinkurl() {
		if($_POST['linkurl']=='http://' || strlen($_POST['linkurl'])<12) return false;
		else return $_POST['linkurl'];
	}
	protected function getcontact() {
		if(empty($_POST['contact'])) return false;
		else return serialize($_POST['contact']);
	}
	protected function getcontent() {
		if(empty($_POST['content'])) return false;
		$tmp = $_POST['content'];
	...
	public function checktype() {
		if(!$_POST['ispic'] && $_POST['content']=='') return false;
		if($_POST['ispic'] && $_POST['picurl']=='') return false;
		return true;
	}
	public function checkcontent() {
		$setting = F('config/module_5');
		if(mb_strlen(strip_tags(str_replace("\r\n","\n",$_POST['content'])),'utf-8') > $setting['cell_contentlen']) return false;
		else return true;
	}
	public function checktitle() {
		$setting = F('config/module_5');
		if(mb_strlen(strip_tags($_POST['title']),'utf-8') > $setting['cell_titlelen']) return false;
		else return true;
	}
}