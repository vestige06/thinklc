...
class InfoAction extends CommonAction {
	...
	public function _filter(&$map) {
		$_query = 'status=1';
		if($_GET['isexpired']==1) $_query .= ' AND (expired=0 OR expired>'.time().')';
		elseif($_GET['isexpired']==2) $_query .= ' AND expired>0 AND expired<'.time();
		if(!empty($_GET['fromtime'])) $_query .= ' AND edittime>'.(strtotime($_GET['fromtime'].' 00:00:00'));
		if(!empty($_GET['totime'])) $_query .= ' AND edittime<'.(strtotime($_GET['totime'].' 23:59:59'));
		$map['_string'] = $_query;
	}
	...
	public function top(){
		...
		$map = $this->_search('Info');
		$_query = 'status=1';
		if($_GET['istotime']==1) $_query .= ' AND toptotime>'.time();
		elseif($_GET['istotime']==2) $_query .= ' AND toptotime<'.time();
		if(!empty($_GET['fromtime'])) $_query .= ' AND toptotime>'.(strtotime($_GET['fromtime'].' 00:00:00'));
		if(!empty($_GET['totime'])) $_query .= ' AND toptotime<'.(strtotime($_GET['totime'].' 23:59:59'));
		$map['_string'] = $_query;
	...
	public function topbuy(){
		...
		if($vo) {
			if($_POST['dosubmit']) {
				...
				if($vo['topstatus'] && $vo['toptotime']>time()) $data['toptotime'] = strtotime("+".$_POST['topdays']." month",$vo['toptotime']);
				...
				if(false !== $result) {
					if($_POST['deduct'] && $_POST['uid']) {
						if($paytype) {
							...
							credit_record($uinfo['uname'], -$credit, $this->adminname, 'ID:'.$_POST['id'].',信息置顶', '自动');
						} else {
							money_add($uinfo['uname'], -$money);
							money_record($uinfo['uname'], -$money, '站内', $this->adminname, 'ID:'.$_POST['id'].',信息置顶', '自动');
						}
					}
	...
	public function delete(){
	    ...
	    if($id) {
			...
			if($MCFG['credit_delete']) {
				//删除非过期信息扣除积分
				$model = D("InfoView");
				$dscore = $model->field('uname,count')->where('Info.id IN ('.$ids.') AND (Info.expired=0 OR Info.expired>='.time().')')->group('Info.uid')->select();
				if($dscore) {
					foreach($dscore as $v) {
