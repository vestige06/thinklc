...
class usermodel {
	...
	function add_bbsuser($username, $password, $email, $uid = 0, $regip = '') {//ThinkLC ADD
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$password =  md5(md5($password));
		$pre = UC_BBS_PRE.'common_';
		$check = $this->db->result_first("SELECT COUNT(*) FROM ".$pre."member WHERE username = '$username'");
		if(!$check) {
			$this->db->query("INSERT INTO ".$pre."member (uid,email,username,password,groupid,regdate,timeoffset) VALUES ('".$uid."','".$email."','".$username."','".$password."','10','".$this->base->time."','9999')");
			$this->db->query("INSERT INTO ".$pre."member_count (uid) VALUES ('".$uid."')");
			$this->db->query("INSERT INTO ".$pre."member_field_forum (uid,customshow) VALUES ('".$uid."','26')");
			$this->db->query("INSERT INTO ".$pre."member_field_home (uid) VALUES ('".$uid."')");
			$this->db->query("INSERT INTO ".$pre."member_profile (uid) VALUES ('".$uid."')");
			$this->db->query("INSERT INTO ".$pre."member_status (uid,regip,lastip,lastvisit,lastactivity) VALUES ('".$uid."','".$regip."','".$regip."','".$this->base->time."','".$this->base->time."')");
			$this->db->query("INSERT INTO ".$pre."onlinetime (uid) VALUES ('".$uid."')");
		}
		return $uid;
	}
